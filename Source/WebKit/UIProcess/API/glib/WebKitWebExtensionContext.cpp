...
#if PLATFORM(WPE)
    case PROP_BACKEND:
        g_value_set_static_boxed(value, context->priv->backend.get());
        break;
#endif
...
static void webkitWebExtensionContextSetProperty(GObject* object, guint propId, const GValue* value, GParamSpec* paramSpec)
{
    ...
    case PROP_BACKEND: {
        ...
        context->priv->backend = backend ? adoptGRef(static_cast<WebKitWebViewBackend*>(backend)) : nullptr;
        break;
    }
#endif
    case PROP_WEB_EXTENSION:
...
#if PLATFORM(WPE)
    /**
     * WebKitWebExtensionContext:backend:
     *
     * The [struct@WebViewBackend] of the view.
     *
     * since: 2.52
     */
...
#if PLATFORM(WPE)
/**
 * webkit_web_extension_context_get_backend:
 * @context: a [class@WebExtensionContext]
 *
 * Get the [struct@WebViewBackend] for any web view generated by this context.
 *
 * Returns: (transfer none): the [struct@WebViewBackend], or %NULL if a backend is not available.
 *
 * Since: 2.52
 */
WebKitWebViewBackend* webkit_web_extension_context_get_backend(WebKitWebExtensionContext* context)
{
    g_return_val_if_fail(WEBKIT_IS_WEB_EXTENSION_CONTEXT(context), nullptr);

    return context->priv->backend.get();
}
#endif
