...
WebKitWebExtension* webkitWebExtensionCreate(const& HashMap<String, String> resources, GError** error)
{
...
WebKitWebExtension* webkitWebExtensionCreateWithBytes(const& HashMap<String, std::span<const uint8_t>> resources, GError** error)
{
    HashMap<String, Ref<API::Data>> newResources;
    for (auto key : resources.keys()) {
        auto data = API::Data::create(resources.get(key));
        newResources.set(key, data);
    }
 ...
 * 
 * Returns: the localized display action label, or %NULL if there
...
const gchar* const * webkit_web_extension_get_requested_permissions(WebKitWebExtension* extension)
{
    ...
    g_ptr_array_add(extension->priv->requestedPermissions.get(), nullptr);
...
const gchar* const * webkit_web_extension_get_optional_permissions(WebKitWebExtension *extension)
{
    ...
    g_ptr_array_add(extension->priv->optionalPermissions.get(), nullptr);
...
GList* webkit_web_extension_get_requested_permission_match_patterns(WebKitWebExtension *extension)
{
    ...
    for (Ref pattern : matchPatternSet)
        returnValue = g_list_prepend(returnValue, webkit_web_extension_match_pattern_ref(webkitWebExtensionMatchPatternCreate(pattern)));
