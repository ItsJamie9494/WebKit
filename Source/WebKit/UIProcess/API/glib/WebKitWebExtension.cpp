...
struct _WebKitWebExtensionPrivate {
    ...
    CString default_locale;
    CString display_name;
    CString display_short_name;
    CString display_version;
    CString display_description;
    CString version;
...
WebKitWebExtension* webkitWebExtensionCreate(HashMap<String, String>   resources,
                                             GError                  **error)
{
    HashMap<String, Ref<API::Data>> newResources;
    for (auto key : resources.keys()) {
        auto string = resources.get(key);
        auto data = API::Data::create(string.utf8().span());

        newResources.set(key, data);
    }

    Ref extension = WebKit::WebExtension::create(WTFMove(newResources));

    if (!extension->errors().isEmpty()) {
        auto internalError = extension->errors().last().ptr();
    ...
    WebKitWebExtension* object = WEBKIT_WEB_EXTENSION(g_object_new(WEBKIT_TYPE_WEB_EXTENSION, NULL));
 ...
 */
gboolean webkit_web_extension_supports_manifest_version(WebKitWebExtension *extension,
                                                        gdouble             manifest_version)
{
    g_return_val_if_fail(WEBKIT_IS_WEB_EXTENSION(extension), FALSE);

    return extension->priv->extension->supportsManifestVersion(manifest_version);
}
 ...
 * 
 * Returns: the default locale, or %NULL if there was no default
 * locale specified.
 */
const gchar* webkit_web_extension_get_default_locale(WebKitWebExtension *extension)
{
    ...
    auto default_locale = priv->extension->defaultLocale();
    if (default_locale.isEmpty())
        return 0;

    priv->default_locale = default_locale.utf8();
    return priv->default_locale.data();
}
 ...
 * 
 * Returns: the localized name, or %NULL if there was no
 * name specified.
 */
const gchar* webkit_web_extension_get_display_name(WebKitWebExtension *extension)
{
    ...
    WebKitWebExtensionPrivate* priv = extension->priv;
    auto display_name = priv->extension->displayName();
    if (display_name.isEmpty())
        return 0;

    priv->display_name = display_name.utf8();
    return priv->display_name.data();
}
...
const gchar* webkit_web_extension_get_display_short_name(WebKitWebExtension *extension)
{
    ...
    WebKitWebExtensionPrivate* priv = extension->priv;
    auto display_short_name = priv->extension->displayShortName();
    if (display_short_name.isEmpty())
        return 0;

    priv->display_short_name = display_short_name.utf8();
    return priv->display_short_name.data();
}
 ...
 * 
 * Returns: the localized display version, or %NULL if there was no
...
const gchar* webkit_web_extension_get_display_version(WebKitWebExtension *extension)
{
    ...
    WebKitWebExtensionPrivate* priv = extension->priv;
    auto display_version = priv->extension->displayVersion();
    if (display_version.isEmpty())
        return 0;

    priv->display_version = display_version.utf8();
    return priv->display_version.data();
}
 ...
 * 
 * Returns: the localized display description, or %NULL if there
...
const gchar* webkit_web_extension_get_display_description(WebKitWebExtension *extension)
{
    ...
    WebKitWebExtensionPrivate* priv = extension->priv;
    auto display_description = priv->extension->displayDescription();
    if (display_description.isEmpty())
        return 0;

    priv->display_description = display_description.utf8();
    return priv->display_description.data();
}
 ...
 * 
 * Returns: the version, or %NULL if there was no
 * version specified.
 */
...
const gchar * const * webkit_web_extension_get_requested_permissions(WebKitWebExtension *extension)
{
    ...
    auto requestedPermissions = extension->priv->extension->requestedPermissions();
    if (!requestedPermissions.size())
        return 0;

    extension->priv->requestedPermissions = adoptGRef(g_ptr_array_new_with_free_func(g_free));
    for (auto permission : requestedPermissions) {
        g_ptr_array_add(extension->priv->requestedPermissions.get(), g_strdup(permission.utf8().data()));
    }
    g_ptr_array_add(extension->priv->requestedPermissions.get(), 0);

    return reinterpret_cast<gchar**>(extension->priv->requestedPermissions->pdata);
}
...
const gchar * const * webkit_web_extension_get_optional_permissions(WebKitWebExtension *extension)
{
    ...
    auto optionalPermissions = extension->priv->extension->optionalPermissions();
    if (!optionalPermissions.size())
        return 0;

    extension->priv->optionalPermissions = adoptGRef(g_ptr_array_new_with_free_func(g_free));
    for (auto permission : optionalPermissions) {
        g_ptr_array_add(extension->priv->optionalPermissions.get(), g_strdup(permission.utf8().data()));
    }
    g_ptr_array_add(extension->priv->optionalPermissions.get(), 0);

    return reinterpret_cast<gchar**>(extension->priv->optionalPermissions->pdata);
}
...
gboolean webkit_web_extension_get_has_commands(WebKitWebExtension *extension)
{
    ...
    return extension->priv->extension->hasOverrideNewTabPage(); //hasCommands is not ported yet
}
...
gboolean webkit_web_extension_get_has_content_modification_rules(WebKitWebExtension *extension)
{
    ...
    return extension->priv->extension->hasOverrideNewTabPage(); //declarativeNetRequestRulesets is not ported yet
}
