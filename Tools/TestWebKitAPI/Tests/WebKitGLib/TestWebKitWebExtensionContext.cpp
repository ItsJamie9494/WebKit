...
#if PLATFORM(WPE)
static inline GRefPtr<WebKitWebExtensionContext> createContext(WebKitWebExtension* extension, GError** error)
{
    if (!Test::s_useWPELegacyAPI)
        return webkit_web_extension_context_new_for_extension(extension, nullptr, error);
    return webkit_web_extension_context_new_for_extension(extension, Test::createWebViewBackend(), error);
}
#else
static inline GRefPtr<WebKitWebExtensionContext> createContext(WebKitWebExtension* extension, GError** error)
{
    return webkit_web_extension_context_new_for_extension(extension, error);
}
#endif
...
static void testDefaultPermissionChecks(Test*, gconstpointer)
{
    ...
    GRefPtr<WebKitWebExtensionContext> context = createContext(extension.get(), &error.outPtr());
    g_assert_no_error(error.get());
    ...
    g_assert_no_error(error.get());
    context = createContext(extension.get(), &error.outPtr());
    g_assert_no_error(error.get());
